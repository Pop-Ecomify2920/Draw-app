...
// Track if RevenueCat should be enabled (has key and not web)
const shouldEnable = !!apiKey && !isWeb;
...
const initializeRevenueCat = (): boolean => {
  ...
  try {
    ...
    console.log(`${LOG_PREFIX} Failed to initialize:`, error);
    return false;
  }
};
...
const guardRevenueCatUsage = async <T>(
  ...
  if (!isInitialized && !initializeRevenueCat()) {
    console.log(`${LOG_PREFIX} ${action} skipped: initialization failed`);
    return { ok: false, reason: "init_failed", error: initializationError };
  }
  ...
  try {
    ...
    // Check for non-std C++ exception or similar native errors
    const errorStr = String(error);
    if (errorStr.includes("non-std") || errorStr.includes("C++ exception")) {
      console.log(`${LOG_PREFIX} ${action} failed with native error - SDK may not be available in this environment`);
...
if (shouldEnable) {
  ...
  setTimeout(() => {
    try {
      ...
      console.log(`${LOG_PREFIX} Deferred initialization failed:`, e);
    }
  }, 0);
}
...
export const isRevenueCatEnabled = (): boolean => {
  return shouldEnable && (isInitialized || !initializationError);
};
