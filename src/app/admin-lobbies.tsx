...
import { ArrowLeft, Users, DollarSign, Calendar, Hash } from 'lucide-react-native';
import { useQuery } from '@tanstack/react-query';
import { AdminService, type AdminLobby } from '@/lib/api/admin';
import * as Haptics from 'expo-haptics';

const LobbyCard = ({ lobby }: { lobby: AdminLobby }) => {
  const statusColor =
    lobby.status === 'drawn'
      ? '#22C55E'
      : lobby.status === 'open'
    ...
    <View className="bg-[#1E3A5F]/60 rounded-xl p-4 border border-[#2E4A6F] mb-3">
      <View className="flex-row items-center justify-between mb-3">
        <View className="flex-1">
          <View className="flex-row items-center mb-1">
            ...
            <Text className="text-white font-semibold text-base ml-2">{lobby.code}</Text>
          </View>
          <Text className="text-white/60 text-sm">Host: {lobby.hostUsername}</Text>
          <View
        ...
        <View className="flex-1">
          ...
          <View className="flex-row items-center">
            ...
            <Text className="text-white font-semibold ml-1">
              ${Number(lobby.prizePool).toFixed(2)}
            </Text>
          </View>
        </View>
        <View className="flex-1">
          <Text className="text-white/60 text-xs mb-1">Members</Text>
          <View className="flex-row items-center">
            <Users size={14} color="#3B82F6" />
            <Text className="text-white font-semibold ml-1">{lobby.memberCount}</Text>
          </View>
        </View>
        <View className="flex-1">
          <Text className="text-white/60 text-xs mb-1">Created</Text>
          <View className="flex-row items-center">
      ...
      <View className="mt-3 pt-3 border-t border-[#2E4A6F]">
        <Text className="text-white/40 text-xs">
          ID: {lobby.id.slice(0, 8)}... • Host ID: {lobby.hostId.slice(0, 8)}...
        </Text>
      </View>
    </View>
  );
...
export default function AdminLobbiesScreen() {
  const router = useRouter();
  const [page, setPage] = useState(1);
  const [filterStatus, setFilterStatus] = useState<string>('');

  const { data, isLoading, isError, error, refetch, isRefetching } = useQuery({
    queryKey: ['admin-lobbies', page, filterStatus],
    queryFn: async () => {
      const response = await AdminService.getLobbies({
        page,
        limit: 20,
        status: filterStatus || undefined,
      });
      if (!response.success) {
        throw new Error(response.error || 'Failed to load lobbies');
      }
    ...
    <View className="flex-1 bg-[#0A1628]">
      <SafeAreaView className="flex-1" edges={['top']}>
        ...
        <View className="flex-row items-center px-4 py-3 border-b border-[#2E4A6F]">
          ...
          <Text className="text-white font-bold text-xl ml-2">Lobby Management</Text>
        </View>
        ...
        >
          {isLoading ? (
            <View className="items-center justify-center py-10">
              ...
              <Text className="text-white/60 mt-4">Loading lobbies...</Text>
            </View>
          ) : isError ? (
            <View className="bg-red-500/20 rounded-xl p-4 border border-red-500/40">
              <Text className="text-red-400">
                {error instanceof Error ? error.message : 'Failed to load lobbies'}
              </Text>
            </View>
            ...
            <>
              <View className="mb-3">
                <Text className="text-white/60 text-sm">
                  Showing {data.lobbies.length} of {data.total} lobbies
                </Text>
              </View>
            ...
            <View className="items-center justify-center py-10">
              <Text className="text-white/60">No lobbies found</Text>
            </View>
          )}
        </ScrollView>
      </SafeAreaView>
